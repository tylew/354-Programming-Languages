...
-- ECons, ENil, EHd, ETl, ELE
-- Part 3: ENil, ECons, EHd, ETl, ELE
subst id s (ECons hd tl) = ECons (subst id s hd) (subst id s tl)
subst id s ENil = ENil
subst id s (EHd lst) = EHd (subst id s lst)
subst id s (ETl lst) = ETl (subst id s lst)
